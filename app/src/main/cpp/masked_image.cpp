...
/*MaskedImage MaskedImage::downsample() const {
    ...
    const auto new_size = cv::Size(size.width / 4, size.height / 4);

    auto ret = MaskedImage(new_size.width, new_size.height);
    for (int y = 0; y < size.height - 1; y += 4) {
        for (int x = 0; x < size.width - 1; x += 4) {
            int r = 0, g = 0, b = 0, ksum = 0;

            for (int dy = -kernel_size.height / 4 + 1; dy <= kernel_size.height / 4; ++dy) {
                for (int dx = -kernel_size.width / 4 + 1; dx <= kernel_size.width / 4; ++dx) {
                    int yy = y + dy, xx = x + dx;
                    if (yy >= 0 && yy < size.height && xx >= 0 && xx < size.width && !is_masked(yy, xx)) {
                        auto source_ptr = get_image(yy, xx);
                        int k = kernel[kernel_size.height / 4 - 1 + dy] * kernel[kernel_size.width / 4 - 1 + dx];
            ...
            if (ksum > 0) {
                auto target_ptr = ret.get_mutable_image(y / 4, x / 4);
                target_ptr[0] = r, target_ptr[1] = g, target_ptr[2] = b;
                ret.set_mask(y / 4, x / 4, 0);
            } else {
                ret.set_mask(y / 4, x / 4, 1);
            }
        }
    }
